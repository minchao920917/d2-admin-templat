...
export function renderNode(h, data, context) {
  const { props } = context
  const cls = ['org-tree-node',`org-tree-node${data.level}`]
  const childNodes = []
...
export function renderLabel(h, data, context) {
  ...
  const cls = ['org-tree-node-label-inner',`org-tree-node-label-inner${data.level}`]
  ...
  labelClassName && cls.push(labelClassName+data.level)
...
export function render(h, context) {
  const { props } = context
  console.log('leave',arrayTreeAddLevel([props.data],0))
  const levelData = arrayTreeAddLevel([props.data],0)[0]
  return renderNode(h, levelData, context)
}
